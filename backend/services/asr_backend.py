...
def normalize_whisper_language(lang: Optional[str]) -> Optional[str]:
    """Map VoiceStudio/BCP-47 language codes to Whisper 2-letter ISO 639-1 language codes."""
    if not lang:
        return None
    ...
    if lower in ("auto", "none", ""):
        return None
    if lower in ("cmn-hans", "cmn-hant", "zh-cn", "zh-tw", "zh-hk", "zh-hans", "zh-hant", "chinese"):
        return "zh"
    if "-" in lower:
...
class ASRBackend(ABC):
    ...
    @abstractmethod
    def transcribe(self, audio_path: str, *, word_timestamps: bool = True, language: Optional[str] = None, **kwargs) -> dict:
...
class FasterWhisperBackend(ASRBackend):
    ...
    def transcribe(self, audio_path: str, *, word_timestamps: bool = True, language: Optional[str] = None, **kwargs) -> dict:
        self._ensure_model()
        lang_arg = normalize_whisper_language(language)
        logger.info(
            "faster-whisper transcribing %s (word_timestamps=%s, language=%s -> %s)",
            audio_path, word_timestamps, language, lang_arg,
        )
...
def deepgram_has_key() -> bool:
    ...
    if os.environ.get("DEEPGRAM_API_KEY"):
        return True
    return _ASR_DEEPGRAM_SECRET_NAME in settings_store.list_secret_names()
...
def probe_deepgram_server(
    ...
    *,
    ...
    """Reachability and auth probe for Deepgram Cloud ASR.

    Checks https://api.deepgram.com/v1/projects with the API key.
    ...
    try:
        with httpx.Client(
            ...
            resp = client.get("https://api.deepgram.com/v1/projects", headers=headers)
    except httpx.TimeoutException as exc:
        out.update(
            status="timeout",
            latency_ms=round((perf_counter() - t0) * 1000.0, 1),
...
def normalize_deepgram_language(lang: Optional[str]) -> Optional[str]:
    """Map VoiceStudio/BCP-47 language codes to Deepgram API supported language codes."""
    if not lang:
        return None
    cleaned = str(lang).strip()
    lower = cleaned.lower()
...
class DeepgramASRBackend(ASRBackend):
    """Cloud-hosted transcription via Deepgram's Nova-2/Nova-3 API.

    No local GPU/VRAM or PyTorch weights required. Supports automatic smart
    formatting, punctuation, timestamps, and speaker diarization.
    """
    id = "deepgram-asr"
    display_name = "Deepgram (Cloud API)"
    gpu_compat = ("cpu",)  # network client only — zero VRAM/GPU needed

    def __init__(self):
        self._api_key = resolve_deepgram_api_key()
        self._model = resolve_deepgram_model() or "nova-2"

    @classmethod
    def is_available(cls) -> tuple[bool, str]:
        if not resolve_deepgram_api_key():
            return False, "Configure Deepgram API key in Model Catalogue → Engines"
        return True, "ready"
    ...
    def transcribe(self, audio_path: str, *, word_timestamps: bool = True, language: Optional[str] = None, **kwargs) -> dict:
        ...
        normalized_lang = normalize_deepgram_language(language)
        if normalized_lang:
        ...
        logger.info("Deepgram ASR transcribing %s (model=%s, language=%s -> %s)", audio_path, self._model, language, normalized_lang)
        try:
            with open(audio_path, "rb") as f:
                content = f.read()
            with httpx.Client(timeout=120.0) as client:
                resp = client.post(url, params=params, headers=headers, content=content)
            if resp.status_code != 200:
                raise RuntimeError(
                    f"Deepgram API error ({resp.status_code}): {resp.text[:300]}"
                )
            data = resp.json()
        except Exception as exc:
            raise RuntimeError(
                f"Deepgram transcription failed: {type(exc).__name__}: {exc}"
            ) from exc

        return self._adapt_response(data)

    @staticmethod
    def _adapt_response(resp: dict) -> dict:
        ...
        if utterances:
            for u in utterances:
                spk_idx = u.get("speaker")
                speaker_label = f"Speaker {spk_idx + 1}" if spk_idx is not None else "Speaker 1"
                seg_words = [
                    {
                        "word": w.get("word", ""),
            ...
            if channels:
                ...
                if words:
                        ...
                        "words": [
                            {
                                ...
                                "end": float(w.get("end", 0.0)),
                                "score": float(w.get("confidence", 1.0)),
                                "speaker": f"Speaker {w.get('speaker', 0) + 1}",
                            }
                            for w in words
                        ],
                        "speaker": "Speaker 1",
                    ...
                    chunks.append({"text": text, "timestamp": (0.0, 0.0)})
                    segments_out.append({"text": text, "start": 0.0, "end": None, "words": []})
        ...
        if channels:
            alt = (channels[0].get("alternatives") or [{}])[0]
            if alt.get("languages"):
                detected_lang = alt["languages"][0]
            elif alt.get("language"):
                detected_lang = alt["language"]

        return {"chunks": chunks, "segments": segments_out, "language": detected_lang}
...
_INSTALL_HINTS: dict[str, str] = {
    ...
    "deepgram-asr": (
        "No install needed — configure a Deepgram API key in "
        "Model Catalogue → Engines. Cloud-hosted ASR via Nova-2 / Nova-3: "
        "ultra-fast transcription, timestamps, and diarization without GPU or RAM footprint."
    ),
