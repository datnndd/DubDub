"""Permanent CI regression-test test sidecar — DO NOT delete.
...
engine (a production sidecar) is installed.
...
Op flow:
    1. on start: sidecar emits {"op":"ready","engine":"test-sidecar"}
...
Test-only ops (only registered when OMNIVOICE_TEST_SIDECAR_MODE=1):
    - {"op":"probe_env"} → echo back the four canonical HF env vars

Test-only crash hook (only when OMNIVOICE_TEST_SIDECAR_CRASH=1): the sidecar will
...
of the test sidecar is that it can spawn under the bare system Python
...
def main() -> int:
    ...
    test_mode = os.environ.get("OMNIVOICE_TEST_SIDECAR_MODE") == "1"
    crash_after_one = os.environ.get("OMNIVOICE_TEST_SIDECAR_CRASH") == "1"

    _send(stdout, {"op": "ready", "engine": "test-sidecar"})
    ...
    while True:
        ...
        if op == "transcribe" and os.environ.get("OMNIVOICE_TEST_SIDECAR_CRASH_NO_REPLY") == "1":
            os._exit(1)
        try:
            if op == "ping":
