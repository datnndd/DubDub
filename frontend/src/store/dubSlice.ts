...
export interface DubSlice {
  ...
  /** Whether to automatically extract speaker audio and clone voices from video (default false). */
  autoVoiceClone: boolean;
  ...
  setAutoVoiceClone: (v: Updater<boolean>) => void;
...
export const createDubSlice: StateCreator<DubSlice, [], [], DubSlice> = (set, get) => ({
  ...
  setAutoVoiceClone: (v) => set((s) => ({ autoVoiceClone: resolve(v, s.autoVoiceClone) })),
