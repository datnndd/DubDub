...
describe('_resolveApiBase', () => {
  it('served (non-dev) → SAME ORIGIN (fixes remote LAN-share CORS)', () => {
  ...
  it('honors VITE_API_PORT in dev', () => {
    const win = { location: { origin: 'http://localhost:3901', hostname: 'localhost' } };
    expect(_resolveApiBase({ DEV: true, VITE_API_PORT: '4000' }, win)).toBe('http://localhost:4000');
  });
  ...
  it('runtime window.__OMNIVOICE_API_BASE__ wins over everything (Docker prebuilt-image override)', () => {
    ...
    // Beats loopback AND VITE_API_URL; trailing slash stripped.
