...
export function transcribeStreamUrl(
  ...
  autoClone?: boolean | null,
  sourceLanguage?: string | null,
): string {
  const base = `${API}/dub/transcribe-stream/${jobId}`;
  const params: string[] = [];
  if (numSpeakers && Number.isFinite(numSpeakers) && numSpeakers > 0) {
    params.push(`num_speakers=${Math.floor(numSpeakers)}`);
  }
  if (autoClone !== undefined && autoClone !== null) {
    params.push(`auto_clone=${autoClone ? 'true' : 'false'}`);
  }
  if (sourceLanguage && sourceLanguage !== 'auto') {
    params.push(`source_language=${encodeURIComponent(sourceLanguage)}`);
  }
