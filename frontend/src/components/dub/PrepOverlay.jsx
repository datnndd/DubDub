...
function PrepOverlay({ stage, progress, onAbort, large = false }) {
  ...
  const body = (
    <>
      ...
      <div
        className={`flex flex-wrap justify-center gap-[var(--space-2)] text-[var(--text-xs)] text-fg-subtle ${large ? 'dub-prep-chips--lg' : ''}`}
      >
        {stages.map((s) => (
          <span
            key={s}
  ...
  return large ? (
    <div className="flex flex-col items-center justify-center gap-[var(--space-4)] dub-prep-overlay--large">
      {body}
    </div>
  ) : (
    <div className="flex flex-col items-center justify-center gap-[var(--space-3)] p-4 max-w-[90%]">{body}</div>
  );
}
